...
describe('TableCellIndexSpacer 组件测试', () => {
  ...
  const renderTableCellIndexSpacer = (props: any = {}, contextValue: any = {}) => {
    ...
    // Simple render without complex context handling
    return render(
      <ConfigProvider>
  ...
  it('应该在前面插入列', () => {
    const mockInsertNodes = vi.spyOn(Transforms, 'insertNodes').mockImplementation(() => {});
    ...
    // 检查是否有插入按钮元素（即使不可见）
    const actionButtons = document.querySelectorAll('.ant-agentic-md-editor-table-cell-index-spacer-action-button');
  ...
  it('应该显示插入列按钮', () => {
    renderTableCellIndexSpacer(
      { columnIndex: 0 }, 
      { deleteIconPosition: { columnIndex: 0 }, setDeleteIconPosition: mockSetDeleteIconPosition }
    );
