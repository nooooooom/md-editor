 ...
 * 缓存已加载的 aceLangs
 */
let aceLangsCache: Set<string> | null = null;
let aceLangsLoader: Promise<Set<string>> | null = null;

/**
 * 异步加载所有 Ace Editor 语言模式
 * 这些导入主要用于注册模式，不需要直接使用返回值
 */
const loadAceModes = async (): Promise<void> => {
  await Promise.all([
 ...
 */
export const getAceLangs = async (): Promise<Set<string>> => {
  if (aceLangsCache) {
    return aceLangsCache;
  }

  if (!aceLangsLoader) {
    aceLangsLoader = (async () => {
      // 先加载所有模式
      await loadAceModes();

      // 然后加载 ext-modelist 来获取语言列表
      const ext = await import('ace-builds/src-noconflict/ext-modelist');
      aceLangsCache = new Set(ext.default.modes.map((m: any) => m.name));
      return aceLangsCache;
    })();
  }

  return aceLangsLoader;
};
 ...
 * 可以在应用启动时调用，提前开始加载
 */
export const preloadAceModes = (): void => {
